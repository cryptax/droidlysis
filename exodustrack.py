...
def get_arguments():
    ...
    parser.add_argument('-i', '--input', help='DroidLysis kit.conf file', action='store', default='./conf/kit.conf')
...
def compare(exodus, droidlysis, verbose=False):
    ...
    for tracker in exodus:
        if tracker['code_signature'] == '':
            ...
            sig = tracker['code_signature'].replace('.','/').strip().strip('/').split('|')[0]

            # search droidlysis patterns
            found = False
            for item in droidlysis:
                if item['pattern'].startswith(sig):
                    if verbose:
                        print("compare(): {} (sig={}) found in {} (pattern={})".format(tracker['name'],sig, item['section'], item['pattern']))
                    found = True
                    break
