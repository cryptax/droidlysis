...
__version__ = '2.3' # Version of DroidLysis

def get_arguments():
    """Read arguments for the program and returns the ArgumentParser"""

    parser = argparse.ArgumentParser(description='''DroidLysis is a Python 
script which processes Android samples. 

1/ It extracts properties from the samples (e.g connects to Internet, roots the phone...). The extracted properties are printed and/or returns as a CSV file.
2/ It helps the analyst begin its reverse engineering of the sample, by performing a first automatic analysis, disassembling, decompiling and a description draft.''', prog='DroidLysis', epilog='Version '+__version__+' - Greetz from Axelle Apvrille')
    ...
    parser.add_argument('--disable-dump', help='do not dump properties for each sample', action='store_true')
...
def process_file(infile, outdir='/tmp/analysis', verbose=False, clear=False, enable_procyon=False, disable_description=False, disable_dump=False, no_kit_exception=False, disable_sql=False):
    """Static analysis of a given file"""

    if os.access(infile, os.R_OK): 
        print "Processing: " + infile + " ..."
        sample = droidsample.droidsample(infile, outdir, verbose, clear, enable_procyon, disable_description, disable_dump, no_kit_exception)
        sample.unzip()
        sample.disassemble()
